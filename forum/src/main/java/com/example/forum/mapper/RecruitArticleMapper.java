...
public interface RecruitArticleMapper {
    ...
    //创建一个帖子
    @Insert("INSERT INTO recruit_article (user_id, update_date, type, direction, content, start_time, end_time, contact, finish, top)" +
    ...
    @Update("UPDATE recruit_article SET update_date = #{update_date}, type = #{type}, direction = #{direction}, " +
            ...
            "finish = #{finish}, top = #{top} WHERE article_id = #{article_id};")
    int updateRecruitArticleByArticleId(@Param("article_id") Integer article_id,@Param("update_date")Date update_date, @Param("type")Integer type,
